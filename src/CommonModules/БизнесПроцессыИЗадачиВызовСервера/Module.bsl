...
Процедура ПринятьЗадачиКИсполнению(Задачи) Экспорт
	...
	Попытка
		...
		Для каждого Задача Из Задачи Цикл
			...
			ИзменитьСтатусДокумента(Задача);
...
Процедура ИзменитьСтатусДокумента(ЗадачаСсылка) Экспорт
	Если ТипЗнч(ЗадачаСсылка.Предмет) = Тип("ДокументСсылка.Заявка") Тогда
		ДокОб = ЗадачаСсылка.Предмет.ПолучитьОбъект();
		Если ЗадачаСсылка.РольИсполнителя = Справочники.РолиИсполнителей.Архитектор Тогда
			ДокОб.Статус = Перечисления.СтатусыЗаявок.ВРазработке;
			ДокОб.Записать();
		ИначеЕсли ЗадачаСсылка.РольИсполнителя = Справочники.РолиИсполнителей.Монтажник Тогда
			ДокОб.Статус = Перечисления.СтатусыЗаявок.НаКоррекции;
			ДокОб.Записать();
		ИначеЕсли ЗадачаСсылка.РольИсполнителя = Справочники.РолиИсполнителей.Производственник Тогда
			ДокОб.Статус = Перечисления.СтатусыЗаявок.ВПроизводстве;
			ДокОб.Записать();
		КонецЕсли; 
	КонецЕсли; 
