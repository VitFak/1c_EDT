...
Функция ФормаВыполненияЗадачи(ЗадачаСсылка, ТочкаМаршрутаБизнесПроцесса) Экспорт
	...
	Результат.Вставить("ИмяФормы", "БизнесПроцесс.ОбработкаЗаявки.Форма.Действие" + "Отработать");//ТочкаМаршрутаБизнесПроцесса.Имя);   //Виктор
	Возврат Результат;
...
Процедура ОбработкаВыполненияПоУмолчанию(ЗадачаСсылка, БизнесПроцессСсылка, ТочкаМаршрутаБизнесПроцесса) Экспорт
	...
	Если ТочкаМаршрутаБизнесПроцесса = БизнесПроцессы.ОбработкаЗаявки.ТочкиМаршрута.Выполнить Тогда
		УстановитьПривилегированныйРежим(Истина);
	...
	ИначеЕсли ТочкаМаршрутаБизнесПроцесса = БизнесПроцессы.ОбработкаЗаявки.ТочкиМаршрута.Проверить Тогда
		УстановитьПривилегированныйРежим(Истина);
		ЗаданиеОбъект = БизнесПроцессСсылка.ПолучитьОбъект();
		ЗаблокироватьДанныеДляРедактирования(ЗаданиеОбъект.Ссылка);
		ЗаданиеОбъект.Выполнено = Истина;
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Запрос.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Запрос.Текст, Метаданные.БизнесПроцессы.ОбработкаЗаявки.ПолноеИмя());
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	БизнесПроцесс = Метаданные.БизнесПроцессы.ОбработкаЗаявки;
	ИмяПроцедуры = "БизнесПроцессы.ОбработкаЗаявки.ОбработатьДанныеДляПереходаНаНовуюВерсию";
	
	ПроблемныхОбъектов = 0;
	ОбъектовОбработано = 0;
	
	Задание = ОбновлениеИнформационнойБазы.ВыбратьСсылкиДляОбработки(Параметры.Очередь, "БизнесПроцесс.ОбработкаЗаявки");
	...
	Параметры.ОбработкаЗавершена = ОбновлениеИнформационнойБазы.ОбработкаДанныхЗавершена(Параметры.Очередь, "БизнесПроцесс.ОбработкаЗаявки");
...
Процедура ОбновитьНаборыЗначенийДоступаЗадания(Параметры, ЗаданиеСсылка) 
	...
	Попытка
		...
		ЭлементБлокировки = Блокировка.Добавить("БизнесПроцесс.ОбработкаЗаявки");
